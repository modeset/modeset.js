var SignatureCanvas = function( canvasElem, debugging ) {
  ...
  var init = function() {
    ...
    _touch_tracker = new MouseAndTouchTracker( canvasElem, createTouchCallback(), false, 'img canvas' );
    clearCanvas();
  };
        ...
        switch( state ) {
          ...
          case MouseAndTouchTracker.state_move :
            ...
            if( _touching && isTouchInCanvas() ) {
              ...
              var curX = _touch_tracker.touchcurrent.x;
              var curY = _touch_tracker.touchcurrent.y;
  ...
  var isTouchInCanvas = function() {
    return true;
    return ( _touch_tracker.touchcurrent.x > 0 && _touch_tracker.touchcurrent.y > 0 && _touch_tracker.touchcurrent.x < _canvas_width && _touch_tracker.touchcurrent.y < _canvas_height );
  };

  var getImageData = function() {
    // remove corner lines before saving data
    _context.clearRect(0, 0, CORNER_LINE, 1);
    _context.clearRect(0, 0, 1, CORNER_LINE);
    _context.clearRect(_canvas_width - CORNER_LINE, 0, CORNER_LINE, 1);
    _context.clearRect(_canvas_width - 1, 0, 1, CORNER_LINE);
    _context.clearRect(0, _canvas_height - CORNER_LINE, 1, CORNER_LINE);
    _context.clearRect(0, _canvas_height - 1, CORNER_LINE, 1 );
    _context.clearRect(_canvas_width - CORNER_LINE, _canvas_height - 1, CORNER_LINE, 1);
    _context.clearRect(_canvas_width - 1, _canvas_height - CORNER_LINE, 1, CORNER_LINE);
  ...
  var clearCanvas = function() {
    ...
    _context.clearRect( 0, 0, _canvas_width, _canvas_height );
    addCornerLines();
  };

  var addCornerLines = function() {
    // add corner lines
    _context.fillStyle = COLOR_CORNERS;
    _context.fillRect(0, 0, CORNER_LINE, 1);
    _context.fillRect(0, 0, 1, CORNER_LINE);
    _context.fillRect(_canvas_width - CORNER_LINE, 0, CORNER_LINE, 1);
    _context.fillRect(_canvas_width - 1, 0, 1, CORNER_LINE);
    _context.fillRect(0, _canvas_height - CORNER_LINE, 1, CORNER_LINE);
    _context.fillRect(0, _canvas_height - 1, CORNER_LINE, 1 );
    _context.fillRect(_canvas_width - CORNER_LINE, _canvas_height - 1, CORNER_LINE, 1);
    _context.fillRect(_canvas_width - 1, _canvas_height - CORNER_LINE, 1, CORNER_LINE);
  };
