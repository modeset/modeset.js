...
# numeric indicator
*/
var TouchScroller = function( element, elementInner, scrollBarContainer, cursor, isPaged, scrollerDelegate, disableElements ) {
	...
	var onEnd = function( touchEvent ) {
      ...
      if( _cur_position[ _axis ] - _speed > 0 || _cur_position[ _axis ] - _speed < _end_position || _doesnt_need_scroll ) {
		...
		if( _is_paged == true ) {
  		...
  		if( pageChanged == true ) {
  		  if( _scroller_delegate ) _scroller_delegate.pageChanged( _page_index );
  		}
  	}
	};
